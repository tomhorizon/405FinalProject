...
REVISIONS MADE BY DYLAN WEIGLEIN (3/16/23):
 - method target_alg() added to class MLX_Cam
"""
...
#                 print('.', end='')
    ...
    def target_alg(self, xrange):
        """!
        @brief    Find target within 24x32 grayscale thermal image
        @details  The targeting algorithm starts with a 24x32 pre-scaled array full of integers corresponding to thermal
                  values. The goal is to aim for the center of a human target. In general, humans have a vertical aspect
                  ratio (they are taller than they are short), and they have higher heat signatures than their surroundings
                    
                  To find the horizontal center of the target, the algorithm indexes through each column within the bounds
                  provided by xrange. For each column, a weighted average is calculated to favor columns with the highest
                  heat values. This method is used to eliminate occurances of a column full of medium-temperature pixels to
                  have a higher value than a column with a few hot pixels. In general, the center column of the target should
                  be the tallest and the hotttest. The horizontal target is set to that location.
                    
                  Within the "hottest" column, the pixel with the highest heat value is selected as the vertical target. Only
                  the bottom 19 pixels are searched. The algorithm indexes from the top to the bottom of the row. If the heat
                  value of one pixel is greater than or equal to the previous maximum, that index is set as the new vertical
                  target.
        @param    xrange: Range of columns to search for target in
        @returns  error_x, error_y: horizontal and vertical distance (in pixels) that the horizontal and vertical targets are
                  from the center.
        """
        threshhold = 5
        sum_old = 0
        avg_old = 0
        # Find the horizontal target
